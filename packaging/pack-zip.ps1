...
Copy-Item $exe (Join-Path $stage "README.exe")
$note = "README.md $($conf.version) - portable`r`nRun README.exe. Requires Windows 10/11 + WebView2 runtime (built into Win11).`r`n100% offline; no install needed."
[System.IO.File]::WriteAllText((Join-Path $stage "USAGE.txt"), $note, (New-Object System.Text.UTF8Encoding($false)))

$zip = Join-Path $out ("README_" + $conf.version + "_x64_portable.zip")
