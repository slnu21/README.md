...
- **v0.6.8 릴리스** — 마크다운 **작성 기능 대폭 확충** 5단위(단축키 도움말 F1 · 문단/목록 서식 단축키 · 자동완성(파일 경로·헤딩 앵커·코드 언어) · 클립보드 이미지 붙여넣기 · 표 도구(Tab 셀 이동·한글 2칸 정렬·TSV 붙여넣기)) + **미리보기 링크 클릭 시 빈 화면 수정**(격리 iframe이 `#앵커`를 페이지 이동으로 처리하던 것). 편집 액션을 `features/editor/actions.ts` **단일 진실원**으로 통합해 키맵·팔레트·우클릭·도움말이 파생되게 하고, **vitest 도입(243개)** — 그동안 임시로 돌리고 버리던 검증 스크립트를 저장소에 남기기 시작한 릴리스다. **Store 제출 완료(2026-07-30) — 인증·게시 대기.**
