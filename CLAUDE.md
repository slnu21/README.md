...
## 현재 상태 (2026-07-17 기준)
- **미릴리스(v0.6.2 이후)** — **파일 열자마자 dirty 표시 수정**(커밋 `6937275`, 머지 `3f9bd57`): 프로그램적 문서 로드(파일 열기·외부 변경 리로드)가 사용자 편집으로 오인돼 dirty가 켜지던 결함. `features/editor`에 **`contentSync` 애노테이션** 도입 → `updateListener`가 표식 붙은 트랜잭션은 `onChange`를 건너뜀 + `lib/tauri.ts` `readFile`에서 **CRLF/CR→LF 정규화**로 마운트 시 불필요한 문서 교체 제거. 로컬 검증(`tsc`·`vite build`) 통과, 신규 경고 0. 변경로그 `[Unreleased]`에 기록.
