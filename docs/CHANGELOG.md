...
### Added
- **한지(Hanji) 테마** — 한국 전통 종이에 먹. 미색 바탕에 순검정이 아닌 **따뜻한 먹빛** 글자를 쓰고, 강조색은 **주사(朱砂) 붉은색** 입니다 — 목록 불릿과 인용 막대가 고서의 권점(圈點)처럼 붉게 찍힙니다. 바탕에는 손으로 뜬 종이에 남는 가로 **발(簾)자국**을 옷게 깔았습니다(확대해도 깨지지 않고 파일 크기는 늘지 않습니다). 번들된 **나눔명조** 글꼴과 잘 어울립니다.
- **전자잉크(E-Paper) 테마** — 이북리더기처럼 **색을 쓰지 않는** 테마입니다. 그림자를 없애 화면을 평평하게 만들고, 색으로 구분할 수 없는 링크는 **밑줄**로 표시합니다. 서식 구분은 크기·굵기·괘선이 맡습니다. 단 하나, **렌더 오류만 붉은색**을 남겼습니다 — 회색 종이에 붉은 펌 자국 하나처럼, 문제를 놓치면 안 되기 때문입니다.
