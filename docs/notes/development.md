...
## 툴바는 중단점이 아니라 **재서** 접는다 — 줄이면 글자가 겹친다

좁은 창에서 "폴더 열기"와 "저장"이 서로 겹쳐 찍혔다. 원인은 두 겹이다.

- **줄어들게 두었다.** `.tgroup { min-width: 0 }` 이면 그룹 상자는 0 까지 눌리지만 안의
  버튼은 자기 최소폭을 지키며 **밖으로 삐져나온다**. 상자끼리는 안 겹치는데 글자는 겹친다 —
  그래서 컨테이너의 `scrollWidth` 로도, 직계 자식의 상자 비교로도 안 잡힌다(둘 다 통과한다).
  고침: `.titlebar > * { flex: none }` — 안 줄이고 **넘치게** 둔다. 넘침은 잴 수 있다.
- **중단점이 틀렸다.** `@media (max-width: 1024px)` 로 라벨을 감추고 있었는데, 라벨을 단
  툴바가 실제로 요구하는 폭은 **1409px(en) · 1347px(ko)** 다(`probe:layout` 실측). 즉
  1025~1409px 구간이 통째로 깨져 있었고, 기본 창 크기 1200px 이 그 한가운데였다.

라벨 폭은 언어·글꼴·배율마다 다르므로 **하나의 px 값으로는 못 맞춘다**. 그래서
`shell/useToolbarFit.ts` 가 `아이템 자연폭 합 + gap + padding > clientWidth` 인지 보고
단계를 내린다(`lib/toolbarFit.ts` 가 순수 판정). 두 가지가 함정이었다:

- **바깥 여백을 빼먹으면 늦게 접힌다.** `.sep` 은 gap 말고 자기 `margin` 도 갖는다(셋이면
  24px). 여백을 안 세면 이미 넘친 툴바를 "아직 들어간다"고 읽는다.
- **관측은 언어가 바뀌면 버려야 한다.** 넘친 폭을 기억해 두어야 경계에서 안 흔들리는데
  (이력현상), 그 기억은 그때의 라벨 폭에 매인 것이다. en 에서 잰 값을 ko 가 물려받으면
  필요 없이 접힌 채로 남는다.
