...
## 20. RTL 라벨 (페르시아어·아랍어 — 문서 방향과 무관)

다이어그램은 **문서의 글 방향 설정과 무관하게 LTR 배치**다(mermaid 는 LTR 배치 엔진이고 측정
스테이지도 LTR). `DIAGRAM_CTX_CSS` 가 `direction:ltr` 을 고정하지 않으면 문서 루트가 RTL 일 때
래퍼가 그 값을 물려받아 **원본 모드의 넓은 차트가 오른쪽 끝에서 시작**하고 스크롤 원점이 뒤집힌다
(SVG 글자 자체는 `unicode-bidi` 없이는 `direction` 을 무시하므로 라벨 상자는 그대로다 — 실측).
프로브는 paper 설정 넷을 문서 루트 RTL 로 띄워 이 갤러리 전체를 다시 잰다. 라벨 안의 RTL 글자는
Chromium 이 묶음 단위로 알아서 오른쪽→왼쪽으로 그린다 — 여기서도 마지막 글자까지 상자 안에 있어야 한다.

```mermaid
flowchart LR
    A[تمام افراد بشر آزاد به دنیا می‌آیند] --> B[مرحبا بالعالم]
    B --> C[혼합 Mixed سلام 123 hello 끝.]
    C --> D[کوتاه]
```
