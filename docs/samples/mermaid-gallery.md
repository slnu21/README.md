...
> **예외: 17번은 고의로 문법이 깨진 블록**이다(오류 표면화 경로 검증) — 여기서만 오류 메시지가 정상이다.
>
> 13~16은 라벨이 도형 안에 정확히 들어가는지 보는 픽스처다. 측정은 앱 문서, 표시는 미리보기
> 문서에서 일어나므로 두 문맥이 어긋나면 **글자가 도형을 넘거나 잘린다** — 그 증상이 보이면 버그.
...
## 17. 고의 문법 오류 (오류 표면화 — 이 블록만 오류가 정상)
