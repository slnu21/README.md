...
**툴바에는 내장 셋만 남는다**(라이트·다크·페이퍼). 그 옆의 네 번째 버튼이 **테마 창**을 연다 —
`shell/ThemePicker.tsx`, 팔레트의 `테마: 테마…` 와 설정 팝오버의 [테마 고르기…] 도 같은 창이다.

**왜 창인가**: 테마는 파일에서 오므로 개수에 상한이 없는데(폴더에 떨어뜨리는 것이 곧
가져오기다) 전부를 44px 툴바에 이어 붙이면 언젠가 다른 컨트롤을 밀어낸다 — 여섯 개가 되자
실제로 그렇게 됐다. **툴바 버튼 수가 테마 수와 무관하다는 것을 `probe:layout` 이 지킨다**
(가짜 테마 셋을 등록해 보고 버튼이 안 늘어나는지 잰다).

지금 테마가 파일에서 온 것이면 **네 번째 버튼이 그 테마의 아이콘을 달고 눌린 상태로** 있다 —
안 그러면 "지금 무슨 테마인지"가 툴바에서 사라진다.

카드를 누르면 **즉시 적용하고 창은 안 닫힌다** — 뒤에 진짜 앱이 그 테마로 서 있는 것이 어떤
견본보다 정확한 미리보기다. 카드의 작은 견본은 테마 자신의 5토큰으로 칠한다(견본용 색을 따로
두면 파일을 고쳤을 때 견본만 옛 색으로 남는다).
...
`%APPDATA%\com.readme.app\themes.jsonc` — SQLite DB 옆. **테마 창 아래쪽**의
[테마 파일 열기] · [테마 폴더 열기] · [테마 다시 불러오기] · [테마 내보내기] 가 입구다
(버튼 툴팁에 해석된 실제 경로가 뜬다). 고르는 자리와 만드는 자리를 한 창에 둔 것은,
갈라 두면 "내가 만든 테마가 어디 있나"를 두 군데서 찾게 되기 때문이다.
...
`template.ts`(themes.jsonc 템플릿·폴더 안내문). 고르는 창은 `shell/ThemePicker.tsx`. Rust 는 `app_paths.rs`(물리 경로 해석)와
